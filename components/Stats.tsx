...
export function GitHub() {
  const { data: githubData, error: githubDataError } = useSWR(
    `/api/github?username=saifmohamedsv`,
    fetcher,
  );
    ...
    >
      ...
      <li className="transition-opacity">
        <Link className="flex items-center gap-3" href="/blog">
          ...
          <div>
            <FlipNumber>
              {youtubeData ? addCommas(youtubeData.viewers) : "0,000"}
            </FlipNumber>
      ...
      <li className="transition-opacity">
        <Link
          className="flex items-center gap-3 no-underline"
          href={"https://www.youtube.com/@" + "saifmohamed_swe"}
        >
