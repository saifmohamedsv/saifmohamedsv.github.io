...
export default function Post({ post, mousePosition }: PostProps) {
  ...
  const showNewBadge =
    Math.abs(new Date(publishDate).getTime() - new Date().getTime()) / (24 * 60 * 60 * 1000) < 30;
    ...
    <li className="group py-3 transition-opacity">
      <div className="transition-opacity">
        {cover_image && mousePosition && (
          ...
          >
            <Image src={cover_image} alt={title} width={imageWidth} height={imageHeight} />
          </motion.div>
        )}
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="min-w-24 relative aspect-square h-24 w-24 drop-shadow-sm md:hidden">
            <Image src={cover_image} alt={title} fill className="rounded object-cover" />
          </div>
        </div>
      </div>
    </li>
